...
\begin{itensq}
\item Qual a nova força, mantida a distância?
...
\begin{itensq}
\item Qual a força resultante sobre $q$?
...
\item Qual o trabalho para levar uma carga de $\SI{5}{\micro\coulomb}$ de A até C?
\end{itensq}
...
\begin{itensq}
\item Qual o potencial criado por $Q$ nesse ponto?
\item Qual o trabalho realizado pela força externa para trazer $q$ (supondo-a
      trazida lentamente)?
\end{itensq}
\dados{$\ke = \SI{9e9}{\newton\meter\squared\per\coulomb\squared}$;
$V_\infty = 0$.}
...
\item Se as cargas forem liberadas simultaneamente, qual a energia cinética total
...
Qual a carga elétrica que atravessa, durante $\SI{10}{\hour}$, a seção de um
...
\begin{itensq}
\item Qual a constante de proporcionalidade $k$ da reta $R = kL$?
...
$A = \SI{0.5}{\milli\meter\squared}$. Qual o comprimento necessário?
...
$R_2 = \SI{3}{\ohm}$; o fio 1 tem o dobro do comprimento do fio 2. Qual a relação
...
\item Se a tensão aplicada for mantida em $\SI{220}{\volt}$, qual a variação
...
$\SI{24}{\watt}$.
\begin{itensq}
\item Qual a corrente que o percorre?
\item Qual o valor de sua resistência?
\end{itensq}
...
\begin{itensq}
\item Qual a corrente que ele consome?
...
\begin{itensq}
\item Qual a corrente do circuito?
\item Qual a potência dissipada em cada resistor?
\end{itensq}
...
\begin{itensq}
\item Qual a corrente que ele exige da rede?
\item Qual a resistência da sua resistência de aquecimento?
...
\begin{itensq}
\item Qual a tensão total fornecida ao circuito?
\item Se a resistência total for $\SI{5}{\ohm}$, qual a corrente?
\end{itensq}
...
\begin{itensq}
\item Qual o menor número de resistores capaz de fornecer essa tensão?
...
$\SI{60}{\ohm}$ alimentada por $\SI{24}{\volt}$. Determine o resistor série
...
\begin{itensq}
\item Determine o resistor série ideal.
...
\item Qual a máxima tensão que pode ser aplicada à associação?
\item Qual a potência máxima que a associação suporta?
\end{itensq}
...
partir do terminal de referência), qual a tensão de saída em vazio?
...
Um capacitor eletrolítico tem ESR (resistência série equivalente) de
...
Em um RLC série, $R=\SI{40}{\ohm}$, $L=\SI{0.1}{\henry}$ e
...
Determine a resistência crítica de um RLC série com $L=\SI{50}{\milli\henry}$ e
...
uma comutação. Sua corrente inicial é $\SI{0.5}{\ampere}$ e o valor final é
...
Um RLC série tem $R=\SI{10}{\ohm}$, $L=\SI{50}{\milli\henry}$ e
...
Para um RLC série com $R=\SI{40}{\ohm}$, $L=\SI{50}{\milli\henry}$ e
...
Calcule $R_{\mathrm{crit}}$ para um RLC série com $L=\SI{0.1}{\henry}$ e
...
Um RLC paralelo possui $R=\SI{200}{\ohm}$, $C=\SI{100}{\micro\farad}$ e
...
paralelo de $R_1=\SI{6}{\kilo\ohm}$ e $R_2=\SI{3}{\kilo\ohm}$. Determine a
constante de tempo.
...
Um RLC série tem $R=\SI{20}{\ohm}$, $L=\SI{0.1}{\henry}$ e
...
Para um RLC série com $R=\SI{100}{\ohm}$, $L=\SI{0.1}{\henry}$ e
...
Um RLC série possui $R=\SI{20}{\ohm}$, $L=\SI{100}{\milli\henry}$ e
...
$\SI{0.2}{\ampere}$. Qual é a energia total armazenada naquele instante?
...
Para um RLC série alimentado por um degrau de $\SI{10}{\volt}$, use
...
$\tau_{\mathrm{med}}=\SI{80}{\milli\second}$. O capacitor foi medido e está
próximo do nominal. Estime a resistência efetiva vista pelo capacitor e proponha
...
em menos de $\SI{0.5}{\second}$. Projete o resistor série mínimo, verifique o
...
$\alpha$, $\omega_d$, $\omega_0$, $\zeta$, $L$ e a resistência série equivalente.
...
Um estágio série RLC usa $L=\SI{20}{\milli\henry}$ e
...
Após uma comutação, um RLC série fica sem fonte independente e possui
...
corretos, estime a resistência série efetiva e diagnostique a discrepância.
\resposta{$R_{\mathrm{efetivo}}\approx\SI{100}{\ohm}$; há cerca de \SI{80}{\ohm}
de resistência série não contabilizada}
...
corrente:
\begin{itensq}
\item Qual a razão entre as induções magnéticas $B$ obtidas nos dois casos?
...
\item Se o campo variar linearmente de $\SI{0.3}{}$ para $\SI{0.1}{\tesla}$ em
      $\SI{0.5}{\second}$, qual a fem induzida?
\end{itensq}
...
\item Qual o valor máximo da fem?
\end{itensq}
