...
pela qual a matéria macroscópica é praticamente neutra: qualquer desequilíbrio
...
\approx \SI{23}{\minute}$ --- um elétron leva quase meia hora para andar um metro!
...
= \SI{12}{\volt} = E_1$. Confere. Este é um bom lembrete: adotar um sentido
...
central, curto-circuitando-o. Ele deixa de conduzir e some do circuito. Sobram
...
$(R_s)^2 \ge 4R_sR_p$, ou seja, $R_s \ge 4R_p$. Aqui, $25 \ge 24$ --- passou
raspando! Se o enunciado pedisse $\Req=\SI{7}{\ohm}$ em paralelo com a mesma soma
...
\stepcounter{questao}
\resposta{(b)}
\resolucao{Direto pela fórmula do divisor:
\[
...
\]
\textbf{Conclusão:} a carga "puxa" a tensão para baixo. Divisores resistivos só
...
i_C=0.
\]
\textbf{Método:} para achar o estado final de qualquer circuito CC com
...
\textbf{Consequência prática:} carregadores eficientes não usam resistor série
...
cai quando $C$ cresce --- razão pela qual a autodescarga de capacitores grandes
...
\resolucao{Em regime permanente, o capacitor mede a tensão do divisor:
...
\resolucao{No paralelo, $\alpha=1/(2RC)=1/[2\cdot200\cdot\pot{100}{-6}]
...
\resolucao{No paralelo, a condição crítica é
...
\resolucao{A, B e C possuem $\omega_0\approx316{,}2$. No série,
...
\SI{3}{\kilo\ohm} adicional. Isso pode vir de resistência de saída da fonte,
...
$E_L=\tfrac12LI_0^2=\SI{9.6}{\milli\joule}$. Com diodo ideal, a descarga vê
aproximadamente o cobre da bobina: $\tau_A=L/60=\SI{2}{\milli\second}$. Com o
...
ringing/sobressinal. Com \SI{180}{\ohm}, $R>R_{\mathrm{crit}}$ e a resposta será
...
\resolucao{Como $R_{\mathrm{crit}}\approx\SI{89.4}{\ohm}$, no RLC série vale
...
saturação:
\begin{itemize}[leftmargin=1.6em,itemsep=1pt,topsep=2pt]
\item a corrente de magnetização dispara (aquecendo o enrolamento) sem ganho
...
para \emph{anular} a magnetização ($B = 0$). Mede a "teimosia" do material em
...
desmagnetização: quanto mais larga a área, mais estável ele é. \emph{O mesmo
...
\end{itemize}
\textbf{Existe, portanto, uma velocidade de força máxima}, acima da qual o freio
