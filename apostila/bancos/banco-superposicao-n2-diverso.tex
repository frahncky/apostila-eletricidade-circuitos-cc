...
\end{circuitikz}}
\resposta{$V_1\approx\SI{9.71}{\volt}$, $V_2\approx\SI{6.29}{\volt}$, $i\approx\SI{1.14}{\ampere}$}
\resolucao{Somando as três respostas parciais obtêm-se $V_1=68/7\approx9{,}71$ V e $V_2=44/7\approx6{,}29$ V. Logo $i=(V_1-V_2)/3=8/7\approx1{,}14$ A.}
\end{questao}

\begin{questao}[2]{}
As duas fontes alimentam uma ponte com dois divisores de mesma razão. Determine por superposição $V_{ab}=V_a-V_b$.
\circuitoq{\begin{circuitikz}[american,scale=.68,transform shape]
\coordinate (l) at (0,2.4); \coordinate (r) at (10,2.4); \coordinate (a) at (5,5.2); \coordinate (b) at (5,-.4);
\coordinate (gl) at (0,-2.5); \coordinate (gr) at (10,-2.5); \coordinate (g) at (5,-2.5);
\draw (gl) to[\fonteV,l^={$\SI{12}{\volt}$}] (l);
\draw (gr) to[\fonteV,l_={$\SI{6}{\volt}$}] (r);
\draw (gl)--(g)--(gr);
...
\resolucao{Os dois caminhos têm a mesma razão $R_{esq}/R_{dir}=1/2$. Para cada fonte isoladamente, $V_a$ e $V_b$ recebem a mesma fração da diferença entre os nós extremos. Assim $V_{ab}^{(1)}=V_{ab}^{(2)}=0$ e a soma permanece nula.}
\end{questao}
