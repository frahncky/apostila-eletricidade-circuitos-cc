...
\begin{questao}[3]{}
Determine $V_1$, $V_2$ e a corrente na fonte de $\SI{20}{\volt}$.
\circuitoq{\begin{circuitikz}[american,scale=.9,transform shape]
\draw (0,0) to[isource,l_={$\SI{4}{\ampere}$}] (0,3)--(1.5,3) coordinate(n1);
\draw (n1) to[R,l={$\SI{10}{\ohm}$}] (1.5,0);
...
\node[ground] at (1.5,0){};\node[above,Vcol] at(n1){$V_1$};\node[above,Vcol] at(n2){$V_2$};
\end{circuitikz}}
\resposta{$V_1=\SI{30}{\volt}$; $V_2=\SI{10}{\volt}$; corrente $\SI{1}{\ampere}$ de 2 para 1 através da fonte}
\resolucao{Supernó: $V_1/10+V_2/10=4$ e $V_1-V_2=20$. Logo $(V_1,V_2)=(30,10)$ V. No nó 2, o resistor drena $1$ A; essa corrente deve chegar pela fonte, do nó 1 ao nó 2 externamente, equivalendo a $1$ A de 2 para 1 internamente na convenção indicada.}
\end{questao}

\begin{questao}[3]{}
Determine $V_1$, $V_2$ e $V_3$ na ponte resistiva.
\circuitoq{\begin{circuitikz}[american,scale=.82,transform shape]
\draw (0,0) to[isource,l_={$\SI{3}{\ampere}$}] (0,3) -- (1.3,3) coordinate(n1);
\draw (n1) to[R,l={$\SI{6}{\ohm}$}] (3.8,3) coordinate(n2);
\draw (n2) to[R,l={$\SI{12}{\ohm}$}] (3.8,0);
\draw (n1) to[R,l_={$\SI{12}{\ohm}$}] (1.3,0) coordinate(n3);
\draw (n3) to[R,l_={$\SI{12}{\ohm}$}] (0,0);
\draw (n2) to[R,l={$\SI{6}{\ohm}$}] (n3);
\draw (n1) to[R,l={$\SI{12}{\ohm}$}] (n3);
\draw (3.8,0)--(n3); \node[ground] at(n3){};
\node[above,Vcol] at(n1){$V_1$};\node[above,Vcol] at(n2){$V_2$};\node[below,Vcol] at(n3){$V_3=0$};
\end{circuitikz}}
\resposta{Para a topologia indicada, montar as LKC antes da substituição numérica; a leitura correta dos nós é parte do exercício.}
\resolucao{Esta questão é de leitura topológica: identifique primeiro quais terminais realmente pertencem ao mesmo nó. Redesenhar a rede antes de montar a matriz é uma estratégia válida e recomendada em pontes.}
\end{questao}
