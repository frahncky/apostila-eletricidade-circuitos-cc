...
\]
\[
E_{mês}=0{,}90(30)=\SI{27}{\kilo\watt\hour};
\qquad
...
\resposta{$P_{média}=\SI{5}{\watt}$}
\resolucao{
\[
P_{média}=P_{pico}\times D=50(0{,}10)=\SI{5}{\watt}.
\]
...
\resposta{$P_{média}=\SI{200}{\watt}$; $E=\SI{66.7}{\watt\hour}$}
\resolucao{A potência média é a \textbf{média ponderada pelo tempo}:
\[
P_{média}=\frac{500(5)+100(15)}{20}=\frac{2500+1500}{20}=\SI{200}{\watt}.
\]
\[
E_{ciclo}=P_{média}\times t_{ciclo}=200\times\frac{20}{60}
=\SI{66.7}{\watt\hour}.
\]
...
\item Analise dois casos: pulsos de $\SI{1}{\milli\second}$ e de
      $\SI{10}{\second}$.
\end{itensq}
\resposta{$P_{média}=\SI{5}{\watt}$; a decisão depende da constante térmica}
\resolucao{(a) $P_{média}=\SI{5}{\watt}$, metade do nominal --- aparentemente
seguro.\\
...
(b)
\[
t=\frac{15{,}00}{88{,}42}=0{,}17\ \text{ano}\approx\SI{2}{\month}.
\]
...
\begin{itensq}
\item Deduza $P_{média}=R\,I_{rms}^{2}$ a partir da definição.
\item Mostre que $I_{rms}\ge I_{média}$, com igualdade apenas para corrente
      constante.
\item Aplique a uma onda quadrada de $\SI{0}{\ampere}$ e
      $\SI{4}{\ampere}$ com ciclo de $\SI{25}{\percent}$.
\end{itensq}
\resposta{$I_{rms}=\SI{2}{\ampere}$ contra $I_{média}=\SI{1}{\ampere}$;
$P=R I_{rms}^{2}$}
\resolucao{(a) Por definição, a potência média em um período $T$ é
\[
P_{média}=\frac{1}{T}\int_0^{T}R\,i^{2}(t)\,\mathrm{d}t
